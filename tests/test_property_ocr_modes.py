...
from pdf_grepper.pdf import loader as loader_mod
...
def test_auto_ocr_triggers_on_sparse_text(monkeypatch):
    ...
    try:
        monkeypatch.setattr(loader_mod, "ocr_image_to_text", lambda img: "OCR TEXT")
...
def test_local_ocr_invoked_for_all_pages(monkeypatch):
    ...
    try:
        monkeypatch.setattr(loader_mod, "ocr_image_to_text", lambda img: "OCR LOCAL")
...
def test_none_ocr_mode_skips_processing(monkeypatch):
    ...
    try:
        monkeypatch.setattr(loader_mod, "ocr_image_to_text", lambda img: "OCR SHOULD NOT RUN")
